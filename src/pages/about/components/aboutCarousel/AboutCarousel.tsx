...
const AboutCarousel: React.FC<AboutCarouselProps> = ({ images }) => {
	...
	return (
		...
		>
			<div className={`aboutCarouselInner ${isMobile ? "mobile" : "desktop"}`}>
				{visibleImages.map((img) =>
					img.id === -1 ? (
						<div key="placeholder" className="aboutCarouselPage placeholder" />
					) : (
						<div key={img.id} className="aboutCarouselPage">
							<Skeleton
								className={`aboutCarouselSkeleton ${
									imageData[img.id]?.loaded ? "hidden" : ""
								}`}
							/>
							<img
								className={`aboutCarouselImage ${
			...
			{totalPages > 1 && (
				<div className="aboutCarouselDotContainer">
					<div
						className="aboutCarouselDotInner"
						ref={dotsRef}
